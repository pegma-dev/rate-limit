...
export async function validateRepository(options = {}) {
  ...
  if (!sameJson(packageDirectories.sort(), ["rate-limit"])) {
    fail(
      "the public workspace inventory must contain only packages/rate-limit",
    );
  }
  if (
...
async function smokeTest(tarball) {
  const directory = await mkdtemp(join(tmpdir(), "pegma-rate-limit-smoke-"));
  try {
    await writeFile(
...
async function verifyPreparedArtifact(options = {}) {
  ...
  const expectedTarball = `pegma-rate-limit-${prepared.package.version}.tgz`;
